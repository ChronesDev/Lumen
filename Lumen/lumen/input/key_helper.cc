...
{
    static std::map<string, ulong> KeyMap_ = { { "Null", 0x00 }, { "LeftButton", 0x01 }, { "RightButton", 0x02 },
        { "Cancel", 0x03 }, { "MiddleButton", 0x04 }, { "ExtraButton1", 0x05 }, { "ExtraButton2", 0x06 },
        { "Back", 0x08 }, { "Tab", 0x09 }, { "Clear", 0x0C }, { "Return", 0x0D }, { "Shift", 0x10 },
        { "Control", 0x11 }, { "Menu", 0x12 }, { "Pause", 0x13 }, { "CapsLock", 0x14 }, { "Kana", 0x15 },
        { "Hangeul", 0x15 }, { "Hangul", 0x15 }, { "Junja", 0x17 }, { "Final", 0x18 }, { "Hanja", 0x19 },
        { "Kanji", 0x19 }, { "Escape", 0x1B }, { "Convert", 0x1C }, { "NonConvert", 0x1D }, { "Accept", 0x1E },
        { "ModeChange", 0x1F }, { "Space", 0x20 }, { "Prior", 0x21 }, { "Next", 0x22 }, { "End", 0x23 },
        ...
        { "N5", 0x35 }, { "N6", 0x36 }, { "N7", 0x37 }, { "N8", 0x38 }, { "N9", 0x39 }, { "A", 0x41 }, { "B", 0x42 },
        ...
        { "X", 0x58 }, { "Y", 0x59 }, { "Z", 0x5A }, { "LeftWindows", 0x5B }, { "RightWindows", 0x5C },
        ...
        { "Subtract", 0x6D }, { "Decimal", 0x6E }, { "Divide", 0x6F }, { "F1", 0x70 }, { "F2", 0x71 }, { "F3", 0x72 },
        ...
        { "OEMCopy", 0xF2 }, { "OEMAuto", 0xF3 }, { "OEMENLW", 0xF4 }, { "OEMBackTab", 0xF5 }, { "ATTN", 0xF6 },
        { "CRSel", 0xF7 }, { "EXSel", 0xF8 }, { "EREOF", 0xF9 }, { "Play", 0xFA }, { "Zoom", 0xFB },
        { "Noname", 0xFC }, { "PA1", 0xFD }, { "OEMClear", 0xFE } };

    static bool ShouldMakeKeyMapReverse_ = true;
    static std::map<ulong, string> KeyMapReverse_ = {};

    static fun MakeKeyMapReverse_()->void
    {
        std::map<ulong, string> m;
        for (var& v : KeyMap_)
        {
            m[v.second] = v.first;
        }
    ...
    global fun FindKeyByKeyName(string value)->Key
    {
        try
        {
            ...
            for (var& k : KeyMap_)
            {
                if (k.first.ToLower() == s) { return (Key)k.second; }
            }
            return Key::Null;
        }
    ...
    global fun StringToKey(string value)->Key
    {
        try
        {
            return (Key)KeyMap_.at(value);
        }
        catch (std::out_of_range ex)
        {
            return Key::Null;
    ...
    global fun KeyToKeyName(Key value)->string
    {
        if (ShouldMakeKeyMapReverse_) MakeKeyMapReverse_();

        try
        {
            return KeyMapReverse_.at((ulong)value);
        }
        catch (std::out_of_range ex)
        {
            return "Null";
